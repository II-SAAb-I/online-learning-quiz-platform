    ...
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($errors)) {
            if (!empty($password)) {
                ...
                $sql = "UPDATE users SET full_name = ?, email = ?, password = ?, role = ?, is_active = ? WHERE id = ?";
                $result = db_query($conn, $sql, "ssssii", [$full_name, $email, $hashed_password, $role, $is_active, $user_id]);
            } else {
                $sql = "UPDATE users SET full_name = ?, email = ?, role = ?, is_active = ? WHERE id = ?";
                $result = db_query($conn, $sql, "sssii", [$full_name, $email, $role, $is_active, $user_id]);
            }
            ...
            if ($result) {
                ...
                set_message('error', 'Failed to update user');
            }
        } else {
